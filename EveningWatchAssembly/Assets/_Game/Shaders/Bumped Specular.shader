...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_normal,TRANSFORM_TEX(node_652.rg, _normal))).rgb;
...
///////// Gloss:
                ...
                float4 node_31 = tex2D(_fuseMapGloss,TRANSFORM_TEX(node_652.rg, _fuseMapGloss));
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_normal,TRANSFORM_TEX(node_653.rg, _normal))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
///////// Gloss:
                float node_67 = pow(1.0-max(0,dot(normalDirection, viewDirection)),_SpecularFresnel);
                float4 node_31 = tex2D(_fuseMapGloss,TRANSFORM_TEX(node_653.rg, _fuseMapGloss));
                float gloss = (node_67*(node_31.a*(_SpecularColor.a*1.5)));
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
